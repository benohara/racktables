...
function formatPortLinkHints ($object_id)
{
	...
	foreach ($linkStatus as $portname => $link_info)
	{
		...
		$visible_part = "<img width=16 height=16 src='?module=chrome&uri=img/${img_filename}'>";
	...
	foreach ($object['ports'] as $port)
		if (! isset ($result[$port['name']]))
			$result[$port['name']]['inline'] = "<img width=16 height=16 src='?module=chrome&uri=img/1x1t.gif'>";
	return $result;
}
